...
class _TabLocationState extends State<TabLocation> {
  ...
  _onLastKnownPressed() async {
    ...
    LocationOutput result = await BackdropGeo.lastKnownLocation();
    if (mounted) {
  ...
  _onCurrentPressed() {
    ...
    _listenToLocation(
        id, BackdropGeo.currentLocation(accuracy: LocationAccuracy.best));
  }

  _onSingleUpdatePressed() async {
    final int id = _createLocation('update', Colors.deepOrange);
    _listenToLocation(
        id, BackdropGeo.singleLocationUpdate(accuracy: LocationAccuracy.best));
  }

  _listenToLocation(int id, Stream<LocationOutput> stream) {
  ...
  _updateLocation(int id, LocationOutput result) {
  ...
  Widget build(BuildContext context) {
    ...
    return new Scaffold(
      appBar: new AppBar(
        title: new Text('Airnmap Backdrop Location'),
      ),
      body: new ListView(
        children: children,
      ),
