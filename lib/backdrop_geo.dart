...
part 'channel/backdecodec.dart';
part 'channel/bdhandle.dart';
part 'channel/location_channel.dart';
part 'channel/values.dart';
part 'data/location.dart';
part 'data/location_output.dart';
part 'data/access.dart';
part 'data/result.dart';
part 'facet_android/location.dart';
part 'facet_android/request.dart';
...
class BackdropGeo {
  ...
  /// See also:
  ///
  ///  * [BackdropResult], the result you can expect from this request.
  static Future<BackdropResult> isLocationOperational({
    LocationAccess permission = const LocationAccess(),
  }) =>
  ...
  ///
  ///
  static Future<BackdropResult> enableLocationServices() =>
  ...
  /// See also:
  ///
  ///  * [LocationAccess], which describes what are the available permissions
  ///  * [BackdropResult], the result you can expect from this request.
  static Future<BackdropResult> requestLocationAccess({
    LocationAccess permission = const LocationAccess(),
    bool openSettingsIfDenied = true,
  }) =>
      _locationChannel.requestLocationAccess(_PermissionRequest(
        permission,
  ...
  /// Location is not guaranteed to be available, and request will fail with [BackdropResultErrorType.locationNotFound] otherwise.
  ...
  ///  * [LocationOutput], the result you can expect from this request.
  ...
  static Future<LocationOutput> lastKnownLocation({
    LocationAccess permission = const LocationAccess(),
  }) =>
  ...
  ///
  /// A single [LocationOutput] will be pushed down the stream, and the stream will complete.
  ...
  /// On timeout, the device will return a less accurate location if available, or [BackdropResultErrorType.locationNotFound].
  ///
  /// See also:
  ///
  ///  * [currentLocation], which provides a better way to get the current one-shot location on both platforms.
  ...
  static Stream<LocationOutput> singleLocationUpdate({
  ...
  ///  * [LocationOutput], the result you can expect from this request.
  static Stream<LocationOutput> currentLocation({
    @required LocationAccuracy accuracy,
    bool inBackground = false,
    LocationAccess permission = const LocationAccess(),
    LocationOptionsAndroid androidOptions =
  ...
  /// You can disable this behaviour by setting `true` for [inBackground].
  ///
  /// [LocationOutput] will be pushed down the stream continuously until the subscription is cancelled.
  /// To stop the ongoing location request, cancel the subscription.
  ///
  /// See also:
  ///
  ...
  static Stream<LocationOutput> locationUpdates({
    @required LocationAccuracy accuracy,
