...
package com.airnmap.backdrop_geo.location
...
import com.airnmap.backdrop_geo.BackdropGeoPlugin
...
class LocationClient(private val context: Context) {
  ...
  val permissionResultListener: PluginRegistry.RequestPermissionsResultListener = PluginRegistry.RequestPermissionsResultListener { id, _, grantResults ->
    if (id == BackdropGeoPlugin.Intents.LocationPermissionRequestId) {
  ...
  val activityResultListener: PluginRegistry.ActivityResultListener = PluginRegistry.ActivityResultListener { id, resultCode, _ ->
    when (id) {
      BackdropGeoPlugin.Intents.LocationPermissionSettingsRequestId -> {
        permissionSettingsCallback?.invoke()
        permissionSettingsCallback = null
        return@ActivityResultListener true
      }

      BackdropGeoPlugin.Intents.EnableLocationSettingsRequestId -> {
...
//        return result != null
  ...
  private suspend fun tryShowSettings(): Boolean = suspendCoroutine { cont ->
    ...
    activity!!.startActivityForResult(intent, BackdropGeoPlugin.Intents.LocationPermissionSettingsRequestId)
  }
  ...
  private suspend fun requestPermission(permission: Permission): Boolean = suspendCoroutine { cont ->
    ...
    ActivityCompat.requestPermissions(activity!!, arrayOf(permission.manifestValue), BackdropGeoPlugin.Intents.LocationPermissionRequestId)
  }
            ...
            is ApiException -> when (exception.statusCode) {
              LocationSettingsStatusCodes.RESOLUTION_REQUIRED -> {
                ...
                try {
                  ...
                  resolvable.startResolutionForResult(activity, BackdropGeoPlugin.Intents.EnableLocationSettingsRequestId)
